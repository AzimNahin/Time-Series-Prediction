...
# Loop through each column and fit an auto_arima model
for column in data.columns:
    ...
    # Forecast future values for 12 months (1 year)
    forecast = model.predict(n_periods=forecast_steps)
    
    # Store the forecasted values
    auto_arima_predictions_full[column] = forecast

# Set the index for the forecast to cover the 2020-2021 period
...
plt.ylabel('CCMEWQI')
plt.title('CCMEWQI Over Time (Auto ARIMA Model)')
plt.grid(True)
plt.tight_layout()
